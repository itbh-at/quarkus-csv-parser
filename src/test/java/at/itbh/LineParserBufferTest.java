...
public class LineParserBufferTest {
    ...
    public void testMultiLineStringInBuffers() {
        ...
        var buffers = new Buffer[] {Buffer.buffer("line one\n"), Buffer.buffer("line two\r\n"),
                Buffer.buffer(""), Buffer.buffer("line three\n"), Buffer.buffer("line four\n")};
        var assertSubscriber = parser.parse(Multi.createFrom().items(buffers)).log().subscribe()
                .withSubscriber(AssertSubscriber.create(4));
        assertSubscriber.assertCompleted().assertItems("line one", "line two", "line three",
                "line four");
    }
